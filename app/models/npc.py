...
class NPC(db.Model):
    """NPC model - functionally identical to Character but standalone"""
    __tablename__ = 'npcs'
    
    id = db.Column(db.Integer, primary_key=True)
    npc_id = db.Column(db.String(80), unique=True, nullable=False, index=True)
    ...
    # NPC-specific fields
    ai_behavior = db.Column(db.String(50), default='passive')  # passive, aggressive, merchant, quest_giver, etc.
    is_hostile = db.Column(db.Boolean, default=False)
    respawn_time = db.Column(db.Integer, default=0)  # seconds, 0 = no respawn
    loot_table = db.Column(db.JSON, default=dict)  # Items dropped on death
    dialogue = db.Column(db.JSON, default=dict)  # Dialogue trees
    faction = db.Column(db.String(50), nullable=True)
    reputation_required = db.Column(db.Integer, default=0)  # Minimum reputation to interact
    is_unique = db.Column(db.Boolean, default=False)  # Cannot respawn if killed
    
    # Relationships
    inventory = db.relationship('Item', backref='owner_npc', lazy='dynamic', 
                               foreign_keys='Item.owner_npc_id')
    equipped_items = db.relationship('Item', backref='equipped_npc', lazy='dynamic', 
                                    foreign_keys='Item.equipped_npc_id')
    ...
    def get_race_data(self):
        """Get full race data for this NPC"""
        if not self.race:
            return None
        return get_race_data(self.race)
    
    def get_wearable_slots(self):
        """Get available equipment slots for this NPC's race"""
        if not self.race:
            return []
        return get_wearable_slots(self.race)
    
    def can_wear_slot(self, slot_name):
        """Check if this NPC can wear items in a specific slot"""
        if not self.race:
    ...
    def get_total_currency(self):
        """Get total currency in copper pieces"""
        return (self.gold * 100 * 100) + (self.silver * 100) + self.copper
    
    def set_currency_from_copper(self, total_copper):
        """Set currency from total copper pieces"""
        self.gold = total_copper // (100 * 100)
        remaining = total_copper % (100 * 100)
        self.silver = remaining // 100
        self.copper = remaining % 100
    
    def __repr__(self):
        return f'<NPC {self.name} ({self.npc_id})>'
